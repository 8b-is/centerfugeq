...
function diabloUpdate(dt) {
  ...
  for (const m of malas) if (!m.got && Math.abs(dpx - m.x) < 0.8 && Math.abs(dpy - m.y) < 0.8) { m.got = 1; collected++; if (collected % 3 === 0) tp++; chAddXp(1); bus.emit('mala', { n: collected }); }
  for (const g of godNodes) if (!g.got && Math.abs(dpx - g.x) < 1 && Math.abs(dpy - g.y) < 1) { g.got = 1; chAddXp(5); if (CH && CH.dim < 9) { CH.dim++; if (!CH.dims.includes(CH.dim)) CH.dims.push(CH.dim); CH.dims.sort((a, b) => a - b); } msg('god-node — dimension ' + (CH ? CH.dim : '?') + ' resonates'); }
  for (const b of BE) {
    if (b.dir !== 0) {
    ...
    if (Math.abs(dpx - b.x) < 0.7 && Math.abs(dpy - b.y) < 0.7) { b.dir = 0; karma++; tp++; chAddXp(2); msg('liberated ' + (BEST[b.t % BEST.length] || 'a poison')); }
  }
...
function newRun() { try { localStorage.removeItem(saveKey); } catch (e) {} collected = 0; karma = 0; tp = 0; lives = 3; lampLit = [0, 0, 0]; alloc.fill(0); alloc[0] = 1; Object.assign(stats, { dash: 1, jump: 1, air: 0, run: 0, magnet: 34, lotus: 8, reach: 40 }); gameOver = false; dead = false; won = false; respawn(); msg('a new run — the stream begins'); }
...
document.addEventListener('keydown', e => {
  ...
  if ((e.key === 'r' || e.key === 'R') && gameOver) { lives = 3; gameOver = false; dead = false; won = false; collected = 0; karma = 0; tp = 0; lampLit = [0, 0, 0]; respawn(); msg('restart — the stream continues'); }
...
function update(dt) {
  ...
  // malas + magnet (tree)
  for (const m of malas) { const mx = m.x * TILE + TILE / 2, my = (LH - m.y) * TILE - TILE / 2; if (!m.got && Math.hypot(px + 19 - mx, py + 22 - my) < stats.magnet) { m.got = 1; collected++; if (collected % 3 === 0) tp++; chAddXp(1); bus.emit('mala', { n: collected }); if (collected % 5 === 0) saveGame(); } }
  // powerups
  ...
  for (const b of BE) {
    ...
    if (px + 38 > ex && px < ex + size * 3 && py + 44 > by - 32 - bob && py < by - bob) {
      if (vy > 260 && py + 40 < by - 8 - bob) { b.dir = 0; karma++; tp++; vy = -430; chAddXp(2); bus.emit('liberation', { name: b.name }); msg('liberated ' + (BEST[b.t % BEST.length] || 'a poison')); }
      else if (b.dir !== 0) { die(); }
    }
  }
