...
<div class="hud"><span>malas <b id="m">0</b>/27 · karma <b id="k">0</b> · talent <b id="tp">0</b> · dash <b id="d">1</b> · mode <b id="mod">0/1</b></span><span>T <b id="tt">0.000</b> · confederates <b id="conf">21</b> · <span id="tip">T = talent tree</span></span></div>
...
for (let c = 0; c < LW; c++) { const v = MN.tiles[c % MN.width]; heights[c] = v === 0 ? LH - 3 : v === 1 ? LH - 4 - (c % 3) : -1; if (heights[c] < 0) heights[c] = -1; }
...
let px = 120, py = 320, vx = 0, vy = 0, onGround = false, coyote = 0, jbuf = 0, jheld = false;
...
function groundedAt(a, b) { return tileAt(a, b + 6) === 1 && tileAt(a + 34, b + 6) === 1; }
function startDash() {
...
function update(dt) {
  if (dead || won || treeOpen || menuOpen) return;
  if (modeT > 0) modeT -= dt;
  if (dashT > 0) {
    dashT -= dt;
  ...
  if (py > LH * TILE) { dead = true; return; }
  ...
  // malas + magnet (tree)
  for (const m of malas) { const mx = m.x * TILE + TILE / 2, my = (LH - m.y) * TILE - TILE / 2; if (!m.got && Math.hypot(px + 19 - mx, py + 22 - my) < stats.magnet) { m.got = 1; collected++; if (collected % 3 === 0) tp++; } }
  // powerups
  ...
  for (const b of BE) {
    ...
    if (px + 38 > ex && px < ex + 32 && py + 44 > by - 32 && py < by) {
      if (vy > 260 && py + 40 < by - 8) { b.dir = 0; karma++; tp++; vy = -430; }
      else if (b.dir !== 0) { dead = true; }
    }
  }
